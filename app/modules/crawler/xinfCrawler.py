...
from app.config import XINF_USERNAME, XINF_PASSWORD, XINF_OUTPUT_DIR
from app.utils.logger import get_logger
from app.modules.dataBaseService.bll.wip_assy import WipAssyBLL
from app.modules.crawler.base import BaseCrawler
from app.modules.excelProcess.supplier.xinfeng_wip_handler import XinfengWipHandler
...
class XinfCrawler(BaseCrawler):
    ...
    def run(self) -> bool:
        ...
        try:
            ...
            # 移动文件,若存在则覆盖
            target_dir = XINF_OUTPUT_DIR.replace("downloads/", "downloads/processed/")
            os.makedirs(target_dir, exist_ok=True)
            target_path = target_dir+f"/{os.path.basename(filepath)}"
            ...
            failed_dir = XINF_OUTPUT_DIR.replace("downloads/", "downloads/failed/")
            os.makedirs(failed_dir, exist_ok=True)
            shutil.move(filepath, failed_dir+f"/{os.path.basename(filepath)}")
            return False
