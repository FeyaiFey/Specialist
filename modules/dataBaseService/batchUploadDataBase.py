...
class HisemiWipAnalyzeService:
    def __init__(self):
        self.scheduler = None
        self.job = None
        self.logger = Logger().get_logger('hisemi_wip_analyze')
        self.excel_folder = r'downloads\pending\封装进度表\池州华宇'
    ...
    def batch_upload_database(self):
        ...
        while database_last_date != (date.today()-timedelta(days=1)):
            ...
            try:
                ...
                self.logger.debug(f"成功处理{database_last_date + timedelta(days=1)}的数据")
                
                # 移动已处理的文件到processed文件夹
                processed_folder = self.excel_folder.replace("pending", "processed")
                if not os.path.exists(processed_folder):
                    os.makedirs(processed_folder)
                processed_file = os.path.join(processed_folder, os.path.basename(file_name_next_day))
                os.rename(file_name_next_day, processed_file)
                self.logger.debug(f"已移动文件到processed文件夹: {processed_file}")
