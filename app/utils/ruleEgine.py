...
class RuleEngine:
    ...
    def apply_rules(self, email_data: Dict) -> Dict[str, Any]:
        ...
        """
        self.logger.info(f"开始匹配规则，邮件主题: {email_data.get('subject', '')}")
        ...
        for rule in self.rules.get('rules', []):
            ...
            if self.check_rule_conditions(email_data, rule):
                self.logger.info(f"邮件匹配规则: {rule_name}")
                return {
        ...
        self.logger.info("未匹配到规则，邮件使用默认规则处理")
        return {
