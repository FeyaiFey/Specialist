...
from app.config import JCET_USERNAME, JCET_PASSWORD, JCET_OUTPUT_DIR
from app.modules.dataBaseService.bll.wip_assy import WipAssyBLL
from app.modules.crawler.base import BaseCrawler
from app.modules.excelProcess.supplier.jcet_wip_handler import JcetWipHandler
...
class JcetCrawler(BaseCrawler):
    ...
    def refresh_token(self) -> bool:
        ...
        try:
            ...
            login_data = {
                "username": JCET_USERNAME,
                "encrypted": False,
                "password": JCET_PASSWORD,
                "validity": -2
            }
    ...
    def run(self) -> bool:
        ...
        try:
            ...
            if df is None:
                ...
                # 移动文件到失败目录
                failed_dir = JCET_OUTPUT_DIR.replace("downloads/", "downloads/failed/")
            ...
            self.close()
            # 移动文件,若存在则覆盖
            target_dir = JCET_OUTPUT_DIR.replace("downloads/", "downloads/processed/")
