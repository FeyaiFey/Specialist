...
from .supplier.yaxinGZ_wip_handler import YaxinGZWipHandler
from .supplier.yaxinZJ_wip_handler import YaxinZJWipHandler
...
class ExcelHandler:
    ...
    SUPPLIER_HANDLERS = {
        ...
        '封装进度表_贵州亚芯': YaxinGZWipHandler,
        '封装进度表_浙江亚芯': YaxinZJWipHandler,
    ...
    def process_excel(self, match_result: Dict) -> Any:
        ...
        try:
            ...
            if not supplier or not category:
                self.logger.error(f"{__name__}:{__file__}:match_result中缺少供应商或类别信息")
                return None
                
            if merge_supplier not in self.SUPPLIER_HANDLERS:
                self.logger.error(f"{__name__}:{__file__}:未找到[{merge_supplier}]的处理器")
                return None
            ...
            if match_result.get('category') == '封装送货单':
                self.logger.debug(f"开始处理[{supplier}]的送货单Excel文件")
                result = handler.process(match_result)
                if result is None:
                    self.logger.warning(f"[{supplier}]的送货单处理未返回数据")
                    return None
                try:
                    utils = SupplierUtils()
                    ...
                    self.logger.error(f"将送货单数据添加到工作进程失败: {str(e)}")
                    return None
